...
static void espnow_callback(const esp_now_recv_info_t* recv_info,
...
static void espnow_callback(const esp_now_recv_info_t *recv_info,
    ...
    if (data_len != sizeof(espnow_packet_t)) {
        ESP_LOGW(TAG, "Wrong packet size: %d, expected: %u",
                 data_len, (unsigned int)sizeof(espnow_packet_t));
        return;
    }
    ...
    (unsigned long)packet.seq, packet.adc[0], packet.adc[1], packet.battery_mv, packet.button);
}