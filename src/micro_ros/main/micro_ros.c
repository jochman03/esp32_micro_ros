...
static int32_t sensors_msg_data[ADC_CHANNEL_COUNT];
...
void micro_ros_init(){
    ...
    xTaskCreate(micro_ros_task, "uros_task", CONFIG_MICRO_ROS_APP_STACK, 
        NULL, CONFIG_MICRO_ROS_APP_TASK_PRIO, NULL);    
}
...
void micro_ros_task(void* arg){
    ...
    RCCHECK(rclc_publisher_init_default(&sensors_publisher, &node, 
        ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int32MultiArray), "esp/sensors"));

    // sensors data static allocation
    sensors_msg.data.data = sensors_msg_data;
    sensors_msg.data.size = ADC_CHANNEL_COUNT;
    sensors_msg.data.capacity = ADC_CHANNEL_COUNT;
    ...
    RCCHECK(rclc_publisher_init_default(&joystick_publisher, &node, 
        ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int32MultiArray), "esp/joystick"));
    ...
    RCCHECK(rclc_publisher_init_default(&button_publisher, &node,
        ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Bool), "esp/button"));

    // Create battery_publisher
    RCCHECK(rclc_publisher_init_default(&battery_publisher, &node,
        ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int32), "esp/battery"));


    // create sensors_timer
    rcl_timer_t sensors_timer;

    RCCHECK(rclc_timer_init_default2(&sensors_timer, &support, 
        RCL_MS_TO_NS(100), sensors_timer_callback, true));

    // create controller_timer
    rcl_timer_t controller_timer;

    RCCHECK(rclc_timer_init_default2(&controller_timer, &support, 
        RCL_MS_TO_NS(100), controller_timer_callback, true));

	// create horn_subscriber
	RCCHECK(rclc_subscription_init_default( &horn_subscriber, &node, 
        ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Bool), "esp/horn"));
    ...
    RCCHECK(rclc_executor_add_subscription(&executor, &horn_subscriber, 
        &horn_msg, &horn_subscription_callback, ON_NEW_DATA));
